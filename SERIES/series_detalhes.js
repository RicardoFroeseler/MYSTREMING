...
// Configurar o poster da série
function setSeriesPoster(posterPath) {
    const posterElement = document.getElementById("moviePoster");
    if (posterPath) {
        posterElement.src = `https://image.tmdb.org/t/p/w500${posterPath}`;
        posterElement.alt = "Poster da Série";
    } else {
        posterElement.src = "poster-placeholder.jpg"; // Um poster padrão caso não exista
        posterElement.alt = "Poster não disponível";
    }
}
...
async function renderSeriesDetails() {
    ...
    // Configurar o poster, background e o player
    setSeriesPoster(seriesDetails.poster_path);
