...
async function loadMovies() {
    try {
        ...
        updateMoviesCount(); // Atualizar o contador de filmes
        extractGenres();
        renderGenres();
        renderMovieList();
        renderPagination();
...
function updateMoviesCount() {
    moviesCount.textContent = `Total de Filmes: ${allMovies.length}`;
}
...
function extractGenres() {
    ...
    genres = [...new Set(allMovies.flatMap(movie => movie.categorias))].map(id => ({
        id,
